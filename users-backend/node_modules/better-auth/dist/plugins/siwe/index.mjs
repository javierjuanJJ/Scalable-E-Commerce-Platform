...
const siwe = (options) => {
	const createSiweNonceEndpoint = (path) => createAuthEndpoint(path, {
		...
		if (!isValidSiweNonce(nonce)) throw APIError.fromStatus("INTERNAL_SERVER_ERROR", {
	...
	return {
		...
		endpoints: {
			...
			verifySiweMessage: createAuthEndpoint("/siwe/verify", {
				...
				}).strict().refine((data) => options.anonymous !== false || !!data.email, {
				...
				try {
					...
					if (!isValidSiweNonce(parsedMessage.nonce)) throw APIError.fromStatus("UNAUTHORIZED", {
					...
					if (!await ctx.context.internalAdapter.consumeVerificationValue(`${SIWE_VERIFICATION_IDENTIFIER_PREFIX}${parsedMessage.nonce}`)) throw APIError.fromStatus("UNAUTHORIZED", {
						message: "Unauthorized: Invalid or expired nonce",
						status: 401,
						code: "UNAUTHORIZED_INVALID_OR_EXPIRED_NONCE"
					});
					const nonce = parsedMessage.nonce;
					const parsedWalletAddress = signedWalletAddressSchema.safeParse(parsedMessage.address);
					const walletAddress = parsedWalletAddress.success ? toChecksumAddress(parsedWalletAddress.data) : null;
					...
					if (!walletAddress || typeof chainId !== "number" || chainId <= 0 || !domainMatches) throw APIError.fromStatus("UNAUTHORIZED", {
						message: "Unauthorized: SIWE message does not match the expected nonce, domain, address, or chain ID",
						status: 401,
						code: "UNAUTHORIZED_SIWE_MESSAGE_MISMATCH"
					});
					...
					if (!user) {
						...
						if (!isAnon && normalizedEmail) {
							...
							let reserved = false;
							try {
								reserved = await ctx.context.internalAdapter.reserveVerificationValue({
									identifier,
									value: walletAddress,
						...
						try {
							...
							if (userEmail !== normalizedEmail || !normalizedEmail) throw error;
							if (!await ctx.context.internalAdapter.findUserByEmail(normalizedEmail)) throw error;
							userEmail = walletEmail;
							user = await createSIWEUser(userEmail);
						} finally {
							if (emailClaimIdentifier) await ctx.context.internalAdapter.consumeVerificationValue(emailClaimIdentifier).catch(() => {});
						}
