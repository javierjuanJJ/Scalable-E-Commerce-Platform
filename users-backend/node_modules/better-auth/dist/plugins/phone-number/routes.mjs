...
const signInPhoneNumber = (opts) => createAuthEndpoint("/sign-in/phone-number", {
	...
	const credentialAccount = await ctx.context.internalAdapter.findCredentialAccount(user.id);
...
async function verifyAndConsumePhoneNumberOTP(ctx, opts, body) {
...
*/
const consumePhoneNumberOTP = (opts) => createAuthEndpoint({
	method: "POST",
...
async function verifyPhoneNumberOTP(ctx, opts, identifier, providedCode) {
	...
	if (parseVerificationAttempts(existing.value.split(":")[1]) >= allowedAttempts) {
		await ctx.context.internalAdapter.deleteVerificationByIdentifier(identifier);
	...
	if (attempts >= allowedAttempts) throw APIError.from("FORBIDDEN", PHONE_NUMBER_ERROR_CODES.TOO_MANY_ATTEMPTS);
...
function parseVerificationAttempts(value) {
...
//#endregion
export { consumePhoneNumberOTP, requestPasswordResetPhoneNumber, resetPasswordPhoneNumber, sendPhoneNumberOTP, signInPhoneNumber, verifyPhoneNumber };
